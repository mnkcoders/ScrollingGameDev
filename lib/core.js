...
class Area {
    ...
    offset(offset) {
        if (offset instanceof Area) {
            return this.offsetArea(offset);
        }
        else if (offset instanceof Vector) {
            return this.offsetVector(offset);
        }
        else if (Number.isInteger(offset)) {
            return this.offsetSingle(offset);
        }
        return this;
    };
    /**
    ...
    /**
     * Divide the area size and position
     * @param {Number} value 
     * @returns {Area}
     */
    reduce( value = 1 ){
    ...
    /**
     * Scale the area size and position
     * @param {Number} value 
     * @returns {Area}
     */
    scale( value = 1 ){
...
class Transform{
     ...
     * @param {Number} alpha 
     */
    constructor(scale = Vector.One(), rotation = 0 , color = Color.White, alpha = 255){
        this._scale = scale || Vector.One();
...
export {Vector,Point,Area,Color,Transform,Component};